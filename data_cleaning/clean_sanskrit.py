...
def transliterate_if_needed(sentence):
    # Assuming no transliteration needed for Sanskrit Devanagari script
    return sentence
...
def clean_sentence(sentence):
    # Devanagari Unicode range U+0900 to U+097F, basic punctuation, digits, spaces allowed
...
def process_text_file_batches(input_path, train_size=500000, val_size=500000):
    ...
    process_sentences(train_sentences, 'sa_train.txt')
    process_sentences(val_sentences, 'sa_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_batches('sa.txt')
