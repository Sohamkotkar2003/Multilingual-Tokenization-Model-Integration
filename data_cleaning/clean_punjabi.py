...
def segment_sentences(text):
    # Split on Punjabi danda (|), period, question mark, exclamation mark
    sentences = re.split(r'[।|.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Gurmukhi script
    return sentence
...
def clean_sentence(sentence):
    # Keep Gurmukhi Unicode range U+0A00–U+0A7F, punctuation, digits, spaces
...
def process_text_file_batches(input_path, train_size=500000, val_size=500000):
    logging.info(f'Starting processing input file: {input_path}')
    train_sentences = []
    val_sentences = []
    lines_read = 0
    ...
    logging.info(f'Completed reading {lines_read} lines.')
    process_sentences(train_sentences, 'pa_train.txt')
    process_sentences(val_sentences, 'pa_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_batches('pa.txt')
