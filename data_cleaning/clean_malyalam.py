...
def segment_sentences(text):
    # Malayalam danda (|), period, question mark, exclamation
    sentences = re.split(r'[.|.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Malayalam
    return sentence
...
def clean_sentence(sentence):
    # Malayalam Unicode block U+0D00-U+0D7F, punctuation, digits, spaces
    allowed_pattern = re.compile(r'[\u0D00-\u0D7F\s.,?!\d]+')
...
def process_text_file_batches(input_path, train_size=500000, val_size=100000):
    ...
    process_sentences(train_sentences, 'ml_train.txt')
    process_sentences(val_sentences, 'ml_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_batches('ml.txt')
