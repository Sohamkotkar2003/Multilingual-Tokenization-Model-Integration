...
def segment_sentences(text):
    # Split on Kannada danda (।), period, question mark, exclamation mark
    sentences = re.split(r'[।.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # Assuming no transliteration needed for Kannada script
    return sentence
...
def clean_sentence(sentence):
    # Keep Kannada Unicode range U+0C80 to U+0CFF, common punctuation, digits, spaces
    allowed_pattern = re.compile(r'[\u0C80-\u0CFF\s.,?!\d]+')
...
def process_text_file_in_batches(input_path, batch_size=500000):
    ...
    def flush_batch(sentences, batch_num):
        output_file = 'kn_train.txt' if batch_num == 1 else 'kn_val.txt'
...
if __name__ == '__main__':
    process_text_file_in_batches('kn.txt')
