...
def segment_sentences(text):
    # Split on Tamil danda, period, question mark, exclamation
    sentences = re.split(r'[।.?!]+\s*', text)
    return [s.strip() for s in sentences if s.strip()]

def transliterate_if_needed(sentence):
    # No transliteration needed for Tamil Unicode
    return sentence
...
def clean_sentence(sentence):
    # Allow Tamil Unicode (U+0B80–U+0BFF), punctuation, digits, spaces
...
def process_text_file_with_sentence_limit(input_path, train_limit=500000, val_limit=500000):
    ...
    process_sentences(train_sentences, 'ta_train.txt')
    process_sentences(val_sentences, 'ta_val.txt')
    logging.info('Processing completed.')

if __name__ == '__main__':
    process_text_file_with_sentence_limit('ta.txt')
