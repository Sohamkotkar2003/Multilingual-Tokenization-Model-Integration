...
def segment_sentences(text):
    # Split on Odia danda (୦/।), period, question mark, exclamation mark
    sentences = re.split(r'[।.?!]+\\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Odia Unicode script
    return sentence
...
def clean_sentence(sentence):
    # Allow Odia Unicode (U+0B00–U+0B7F), punctuation, digits, and spaces
...
def process_text_file_batches(input_path, train_size=500000, val_size=500000):
    ...
    process_sentences(train_sentences, 'or_train.txt')
    process_sentences(val_sentences, 'or_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_batches('or.txt')
