...
def segment_sentences(text):
    # split on Assamese danda (।), period (.), question mark, exclamation mark
    sentences = re.split(r'[।.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # Assuming no transliteration needed for Assamese script, return as is
    return sentence
...
def clean_sentence(sentence):
    # Keep Assamese Unicode range U+0980 to U+09FF, common punctuation, digits, spaces
...
def process_text_file_in_batches(input_path, batch_size=500000):
    ...
    def flush_batch(sentences, batch_num):
        output_file = 'as_train.txt' if batch_num == 1 else 'as_val.txt'
...
if __name__ == '__main__':
    process_text_file_in_batches('as.txt')
