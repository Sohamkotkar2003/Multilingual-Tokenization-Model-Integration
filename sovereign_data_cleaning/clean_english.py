...
def segment_sentences(text):
    # Basic sentence segmentation on common English punctuation marks
    sentences = re.split(r'[.!?]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # English no transliteration needed
    return sentence
...
def clean_sentence(sentence):
    # Keep basic English characters, punctuation, digits, and spaces
...
def process_text_file_in_batches(input_path, batch_size=500000):
    ...
    def flush_batch(sentences, batch_num):
        output_file = 'en_train.txt' if batch_num == 1 else 'en_val.txt'
...
if __name__ == '__main__':
    process_text_file_in_batches('en.txt')
