...
def segment_sentences(text):
    # Split on period, question mark, exclamation, danda (।)
    sentences = re.split(r'[।.?!]+\\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Meitei Mayek here
    return sentence
...
def clean_sentence(sentence):
    # Adjusted for Meitei Mayek block (U+ABC0–U+ABFF), punctuation, digits, spaces
    allowed_pattern = re.compile(r'[\uABC0-\uABFF\s.,?!\d]+')
...
def process_text_file_split_in_half(input_path):
    ...
    def flush_batch(sentences, batch_num):
        output_file = 'mni_train.txt' if batch_num == 1 else 'mni_val.txt'
    ...
    # In case of any remaining sentences
...
if __name__ == '__main__':
    process_text_file_split_in_half('mni.txt')
