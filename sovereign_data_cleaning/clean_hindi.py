...
def segment_sentences(text):
    # split on Hindi danda (।), period, question mark, exclamation mark
    sentences = re.split(r'[।.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # Assuming no transliteration needed for Hindi Devanagari script
    return sentence
...
def clean_sentence(sentence):
    # Keep Devanagari Unicode range U+0900 to U+097F, common punctuation, digits, spaces
...
def process_text_file_in_batches(input_path, batch_size=500000):
    ...
    def flush_batch(sentences, batch_num):
        output_file = 'hi_train.txt' if batch_num == 1 else 'hi_val.txt'
...
if __name__ == '__main__':
    process_text_file_in_batches('hi-1.txt')
