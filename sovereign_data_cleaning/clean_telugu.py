...
def segment_sentences(text):
    # Split on Telugu danda (|), period, question mark, exclamation
    sentences = re.split(r'[।|.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Telugu Unicode script
    return sentence
...
def clean_sentence(sentence):
    # Telugu Unicode block U+0C00–U+0C7F, punctuation, digits, spaces allowed
    allowed_pattern = re.compile(r'[\u0C00-\u0C7F\s.,?!\d]+')
...
def process_text_file_with_sentence_limit(input_path, train_sentence_limit=500000, val_sentence_limit=500000):
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for sent in segmented:
                if len(train_sentences) < train_sentence_limit:
                    train_sentences.append(sent)
                elif len(val_sentences) < val_sentence_limit:
                    val_sentences.append(sent)
                else:
                    # Reached sentence limits; stop reading more
                    break

            # Stop reading lines if both sentence limits reached
            if len(train_sentences) >= train_sentence_limit and len(val_sentences) >= val_sentence_limit:
    ...
    process_sentences(train_sentences, 'te_train.txt')
    process_sentences(val_sentences, 'te_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_with_sentence_limit('te.txt')
