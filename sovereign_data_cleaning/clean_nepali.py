...
def segment_sentences(text):
    # Split using Nepali danda (।), period, question mark, exclamation mark
    sentences = re.split(r'[।.?!]+\s*', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    return sentences

def transliterate_if_needed(sentence):
    # No transliteration needed for Nepali Devanagari script
    return sentence
...
def clean_sentence(sentence):
    # Nepali: Devanagari Unicode range U+0900–U+097F, punctuation, digits, spaces
...
def process_text_file_batches(input_path, train_size=500000, val_size=500000):
    ...
    process_sentences(train_sentences, 'ne_train.txt')
    process_sentences(val_sentences, 'ne_val.txt')
    logging.info('Processing complete.')

if __name__ == '__main__':
    process_text_file_batches('ne.txt')
